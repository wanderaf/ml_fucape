...
def run():
    st.title("🔍 SVM – Classificação Binária com Rolling Window de 12 Meses")
    st.info("Modelo classificador com rebalanceamento mensal, rolling window e filtro de confiança baseado em probabilidade.")
    ...
    # reproduz exatamente a lógica SVM para qualquer lista de meses (treino ou teste)
    def _build_curve_svm(df_all, meses_periods, features, prob_thresh=0.55, start_capital=100000.0):
        rows = []
        ...
        for mes in meses_periods:
            ...
            modelo = SVC(kernel='rbf', probability=True, C=1.0, gamma='scale', random_state=42)
    ...
    with st.spinner("🔄 Executando simulação..."):
        ...
        # === Índices OOS (como já tinha) + NOVOS (MDD/Overfit) ===
        ...
        df_train_curve = _build_curve_svm(df, meses_train, features)
        df_test_curve  = _build_curve_svm(df, meses_test,  features)
    ...
    # capital OOS
    df_result.reset_index()[["Data", "Capital"]].to_csv("data/results/svm_capital.csv", index=False)

    # capital treino/teste p/ overfit
    if 'df_train_curve' in locals() and not df_train_curve.empty:
        df_train_curve[["Data", "Capital"]].to_csv("data/results/svm_capital_train.csv", index=False)
    if 'df_test_curve' in locals() and not df_test_curve.empty:
        df_test_curve[["Data", "Capital"]].to_csv("data/results/svm_capital_test.csv", index=False)

    # índices (usa TESTE/OOS para o principal)
    with open("data/results/svm_indices.txt", "w") as f:
        f.write(f"{sharpe_te if pd.notna(sharpe_te) else np.nan},{sortino_te if pd.notna(sortino_te) else np.nan}")
    with open("data/results/svm_indices_train.txt", "w") as f:
        f.write(f"{sharpe_tr if pd.notna(sharpe_tr) else np.nan},{sortino_tr if pd.notna(sortino_tr) else np.nan}")
    with open("data/results/svm_indices_test.txt", "w") as f:
        f.write(f"{sharpe_te if pd.notna(sharpe_te) else np.nan},{sortino_te if pd.notna(sortino_te) else np.nan}")

    with open("data/results/svm_overfit_ratio.txt", "w") as f:
        f.write(f"{overfit_ratio}")
    with open("data/results/svm_max_drawdown_test.txt", "w") as f:
        f.write(f"{mdd_te}")
    ...
    df_treino_shap = df[
        (df["Date"] >= data_inicio_shap) &
        (df["Date"] < data_ref_shap) &
        (df["Volume"] > 500000)
    ].copy()
    ...
    modelo_shap = SVC(kernel='rbf', probability=True, C=1.0, gamma='scale', random_state=42)
    ...
    # KernelExplainer com amostra de background
